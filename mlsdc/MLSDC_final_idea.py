...
class FAS_correction(penning_trap):
    ...
    def SDC_method(self, U, level, tau):
        ...
        for m in range(M):
            for j in range(M+1):
                ...
                f=self.build_f(F[j], U['vel'][j], self.params.dt*level.coll.nodes[j-1])
                integral.pos[m] += self.params.dt*(self.params.dt*(level.SQ[m+1,j]-level.Sx[m+1, j])*f)

                integral.vel[m] += self.params.dt* (level.S[m+1, j]-level.ST[m+1, j]) * f
        ...
        for m in range(0, M):
            ...
            U['vel'][m+1]=self.Boris_solver(ck, self.params.dt*np.diag(level.QI)[m+1], F[m], F[m+1], U['vel'][m])
    ...
    def SDC_iteration(self, Kiter):
        ...
        for ii in range(Kiter):
            U_coarse, tau=self.tau_correction(U, level_coarse=self.coarse, level_fine=self.fine)
            
            pdb.set_trace()
            
            U_fine=self.SDC_method(U, level=self.fine)
